                ...
                getTrProps={(state, rowInfo, column) => {
                    return {
                        ...
                        onClick: (e, handleOriginal) => {
                            if (rowInfo) {
                                props.history.push('/admin/focus-produits/' + rowInfo.original.id);
                            }
                        }
                    }
                }}
                ...
                columns={[
                    {
                        ...
                        Cell: row => (
                            row.original.produit && row.original.produit.featuredImageId ? (
                                <img className="w-full block rounded" src={FuseUtils.getUrl() + row.original.produit.featuredImageId.url} alt="" />
                            ) : (
                                    <img className="w-64 block rounded" src="assets/images/ecommerce/product-image-placeholder.png" alt="" />
                                )
                    ...
                    },
                    {
                        ...
                        Cell: row => (
                            row.original.produit ? (
                                row.original.produit.reference
                            ) : (
                                    'N/A'
                                )
                        )
