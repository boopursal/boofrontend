...
import { Grid, InputAdornment, IconButton, Icon, Typography, Chip, Box } from '@material-ui/core';
...
function Activites(props) {
    ...
    const { commande, selected, handleChipChange, handleDeleteActivite, handleAddSuggestion, handleChangeAutre } = props;

    return (
        <Formsy>
            {
                selected.offre &&
                <Box display="flex" alignItems="center" width="50%" className="mb-16 m-auto">
                    <Box minWidth={45}>
                        Activités
                                </Box>
                    <Box width="100%" mr={1} ml={1}>
                        <BorderLinearProgress variant="determinate" value={((selected.sousSecteurs.length + selected.suggestions.length) / selected.offre.nbActivite) * 100} />
                    </Box>
                    <Box minWidth={35}>
                        <Typography variant="body2" color="textSecondary">
                            {`${(selected.sousSecteurs.length + selected.suggestions.length)}/${selected.offre.nbActivite}`}
                        </Typography>
                    </Box>
                </Box>
            }
                                ...
                                InputProps={{
                                    endAdornment:
                                        <InputAdornment position="end">
                                            <IconButton
                                                color="secondary"
                                                disabled={!selected.autreActivite || selected.autreActivite.length < 2}
                                                onClick={(ev) =>
            ...
            <Grid container spacing={4} className="items-start">
                ...
                <Grid item xs={12} sm={6} >
                    <Typography className="mb-16" variant="h6"><span className={classes.border}>Activités choisies</span></Typography>
                    {
                ...
                <Grid item xs={12} sm={6}>
                    <Typography className="mb-16" variant="h6"><span className={classes.border}>Activités suggérées</span></Typography>
                    {
                        selected.suggestions.map((item, index) => (
                            <Chip
                                ...
                                onDelete={() => handleDeleteActivite(item.sousSecteur, 'suggestions')}
                                className="mr-8 mb-8"
                            />
                        ))
                    }
