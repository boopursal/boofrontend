...
function Duree(props) {
    ...
    const { durees, loading, selected, onChange, handleGetMontantPerMonth, handlegetMontantPerYear } = props;
            ...
            <Grid container spacing={4}>
                {
                    durees && durees.map((item) => (
                        <Grid item xs={12} key={item['@id']} md={6}>
                            <div onClick={() => onChange(item)} className={clsx("rounded rounded-md relative flex items-center justify-center p-24 h-200 w-full border-solid border-4 cursor-pointer hover:border-blue", selected.duree && (selected.duree['@id'] === item['@id'] ? " border-blue" : " border-gray-400"))}>
                                <div className="mr-16">
                                    {
                                        selected.duree && (selected.duree['@id'] === item['@id'] ?
                                ...
                                <div className="flex flex-col items-start">
                                    ...
                                    <Typography variant="caption">
                                        ...
                                        {
                                            selected.currency === 'MAD' ? ' MAD HT par an' : " $ par an"
                                        }
                                    </Typography>
                                </div>
                                {
